    ...
    <div className="flex gap-1 overflow-x-auto">
      {[
        ...
        <button
          ...
          className={`py-2 px-4 rounded-xl text-sm font-semibold transition-all shrink-0 flex items-center gap-2 ${
            active === t.id
        ...
        >
          ...
          {t.badge > 0 && (
            <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${
              active === t.id ? 'bg-teal-600' : 'bg-gray-200 text-gray-600'
            }`}>{t.badge}</span>
          )}
        </button>
      ))}
...
export default function ClinicSchedulerDashboard({ onSelectPatient, onNavigate, onClose }) {
  ...
  const [selectedDay,  setSelectedDay]  = useState(null)  // 1-5 | null
  ...
  // Click en día del mes → abre DayView con las citas de ese día
  function handleMonthDayClick(dayOfWeek) {
    setSelectedDay(dayOfWeek)
  }

  // Desde DayView → abrir modal con día pre-seleccionado
  function handleNewFromDay(dayOfWeek) {
    setSelectedDay(null)
    setModal({ _preselectedDay: dayOfWeek })
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {modal && (
        <AppointmentModal
          appointment={modal === 'new' || modal?._preselectedDay ? null : modal}
          preselectedDay={modal?._preselectedDay ?? null}
          appointments={appointments}
      ...
      {/* DayView — vista de un día desde el mes */}
      {selectedDay && (
        <DayView
          ...
          onEdit={(appt) => { setSelectedDay(null); setModal(appt) }}
          onAttendance={handleAttendance}
          onStartSession={handleStartSession}
        />
      )}
